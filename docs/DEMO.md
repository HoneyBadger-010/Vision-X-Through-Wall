...
The on-site round gives ~24 hours. The goal is **one honest vertical slice** that proves the thesis end-to-end, not a broad-but-faked system. Genuinely sensing a living person **through a solid wall** — no line of sight — makes the demo *real* rather than simulated.

## The one thing to prove

> A firefighter learns there is a **living person in the next room — through a solid wall — roughly where, and that they're breathing — before entering**, hands-free.

## Scope of the slice

| Tier | In the demo | Out of scope (slide / stub) |
|------|-------------|------------------------------|
| **Node** (UNO Q + 1 IR-UWB) | presence + distance + breathing **through a solid wall**, on-device | multi-victim, antenna array, gas/thermal fusion |
...
1. **Setup** — a person hidden in the **next room**, behind a solid (non-metallic) wall, breathing normally. *(Optional: a smoke machine to show RF is unaffected.)*
2. **Advance** — the "firefighter" approaches the wall with an obscured visor. The node reports *motion / direction* ("contact, that way") while moving.
3. **Pause at the wall** — the firefighter stops for a few seconds. This still **dwell** is when breathing is confirmed (the detect-then-confirm workflow).
4. **The cue** — hands-free audio into the comms: *"Living person detected, ~2 m, behind this wall, breathing."*
...
- [ ] CNN classifier loaded (pretrained on public data); **on-site fine-tune samples captured** through the actual wall.
...
- [ ] Two rooms split by a solid (non-metallic) wall; person in the next room staged; through-wall detection rehearsed; optional smoke machine.
...
A judge watches a firefighter detect a living person **through a solid wall**, ending in a spoken, correct *"alive, behind this wall, ~2 m, breathing"* — and sees the same contact appear on a command screen. Everything else in the design is the story of how this scales; the slice is the proof that it works.
